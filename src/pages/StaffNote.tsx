...
const StaffNote = (props: any) => {
  ...
  useEffect(() => {
    async function fetchData() {
      if (data) setStaffNote(data.staffNotesByUser.map((item: any) => item));
    }
    fetchData();
  }, [data]);
  const handleClose = () => {
  ...
  const handleDelete = (selected: Array<any>) => {
    ...
    try {
      ...
      enqueueSnackbar("Delete Successfully", AlertSuccessProp as any);
    } catch (e) {
      enqueueSnackbar("Error occurred", AlertErrorProp as any);
    } finally {
      setIsLoading(false);
    }
  };
